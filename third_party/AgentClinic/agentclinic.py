...
def query_model(model_str, prompt, system_prompt, tries=30, timeout=20.0, image_requested=False, scene=None, max_prompt_len=2**14, clip_prompt=False):
    """
    Query a model by alias. Supports OpenAI Chat (via OpenAI v1 client), Anthropic, Replicate, and HF stubs.

    OpenAI config (API key, base URL, default model) can be set via:
    - CLI (see main()); or
    - Environment variables: OPENAI_API_KEY, OPENAI_BASE_URL, AGENTCLINIC_OPENAI_MODEL/OPENAI_MODEL.
    """
    if model_str not in [
        "gpt4", "gpt3.5", "gpt4o", 'llama-2-70b-chat', "mixtral-8x7b",
        "gpt-4o-mini", "llama-3-70b-instruct", "gpt4v", "claude3.5sonnet", "o1-preview"
    ...
    def _get_openai_client():
        ...
        if '_OPENAI_CLIENT' not in globals() or _OPENAI_CLIENT is None:
            ...
            base_url = os.getenv("AGENTCLINIC_OPENAI_BASE_URL") or os.getenv("OPENAI_BASE_URL") or os.getenv("OPENAI_API_BASE") or "https://api.bianxie.ai/v1"
            _OPENAI_CLIENT = OpenAI(api_key=api_key, base_url=base_url)
        return _OPENAI_CLIENT

    def _resolve_openai_model(alias: str, wants_image: bool = False) -> str:
        # Allow an override for the default model
        override = os.getenv("AGENTCLINIC_OPENAI_MODEL") or os.getenv("OPENAI_MODEL")
        if override:
            return override
        # Sensible defaults for aliases
        if alias in ("gpt4", "gpt4o"):
            return "gpt-4o"
        if alias == "gpt-4o-mini":
            return "gpt-4o-mini"
        if alias == "gpt3.5":
            return "gpt-3.5-turbo"
        if alias == "gpt4v" or wants_image:
            # Modern multimodal default
            return "gpt-4o"
        if alias == "o1-preview":
            # Keep existing pinned default
    ...
    for _ in range(tries):
        ...
        try:
            # --- OpenAI Chat backends (consolidated) ---
            if model_str in ["gpt4", "gpt3.5", "gpt4o", "gpt-4o-mini", "gpt4v", "o1-preview"]:
                client = _get_openai_client()
                if image_requested and scene is not None:
                    messages = [
                ...
                model_name = _resolve_openai_model(model_str, wants_image=image_requested)
...
def main(api_key, replicate_api_key, inf_type, doctor_bias, patient_bias, doctor_llm, patient_llm, measurement_llm, moderator_llm, num_scenarios, dataset, img_request, total_inferences, anthropic_api_key=None, openai_api_base=None, openai_default_model=None):
    # Configure OpenAI via environment for the new v1 client
    if api_key:
        os.environ["OPENAI_API_KEY"] = api_key
    # Allow base URL override (default remains previous project default)
    if openai_api_base:
        os.environ["OPENAI_BASE_URL"] = openai_api_base
    else:
        os.environ.setdefault("OPENAI_BASE_URL", "https://api.bianxie.ai/v1")
    # Allow default model override
    if openai_default_model:
        os.environ["AGENTCLINIC_OPENAI_MODEL"] = openai_default_model
...
if __name__ == "__main__":
    ...
    parser.add_argument('--openai_api_base', type=str, required=False, help='OpenAI API Base URL (e.g., https://api.openai.com/v1)')
    parser.add_argument('--openai_default_model', type=str, required=False, help='Default OpenAI chat model (overrides gpt4/gpt4o aliases)')
